...
In the current state of the Venture Factory, LiNKautowork is not only an architectural concept; it is a concrete reference implementation whose repository supersedes conflicting descriptions elsewhere. That implementation is located at `/Users/linktrend/Projects/LiNKautowork`. In this repo, `automations/templates/` is treated as the canonical source of truth for workflow definitions, `automations/live/dev` and `automations/live/prod` serve as exported evidence of deployed state, and an explicit lifecycle state machine (`draft -> dev_tested -> qa_approved -> ops_approved -> prod_deployed -> deprecated -> archived`) governs promotion and production readiness.

The reference implementation also makes a crucial architectural decision explicit: n8n is not exposed directly as an ungoverned automation surface. Instead, a dedicated gateway service (`gateway/`) acts as the policy and security front door. In that model, inbound workflow execution is gated behind signed ingress requirements (including HMAC signatures, timestamps, and nonces to prevent replay), tenant and lineage validation, and just-in-time secret retrieval from Google Secret Manager. Only requests that satisfy these controls are permitted to reach the underlying n8n webhooks. This means “workflow execution” inside LiNKautowork is treated as a governed system transaction rather than an ad hoc automation trigger.

Operational observability and interoperability are also part of the implemented truth. The gateway writes canonical execution telemetry into Supabase through controlled interfaces so that workflow runs are auditable and reconstructable, and it publishes events onto the shared event plane (NATS) using canonical `aios.*` subjects with an optional internal mirror taxonomy. The reference implementation further encodes the daily ritual windows (08:00 strategic, 10:45 operational pulse, 14:45 quality) as first-class automation outputs. This is significant because it makes governance cadence an automation responsibility rather than a manual reporting habit, and it ensures the same factory-wide decision rhythm is maintained even when execution is distributed.

Finally, the current MVO posture is explicitly internal-only: a canonical internal tenant UUID is enforced in the contract to prevent accidental cross-tenant execution (the current machine-contract tenant is `00000000-0000-0000-0000-000000000001`). This does not eliminate the broader multi-tenant design intent described later in this document, but it establishes that the present operational baseline is a strictly controlled internal tenant with expansion to broader tenancy treated as a later phase of commercialization and governance hardening.
...
In the current reference implementation, this integration is not left to informal conventions. The LiNKautowork gateway defines an explicit ingress envelope and security header set. Workflow ingress requires lineage and identity fields (including tenant context, mission and task lineage, run identifiers, and DPR identity), and it requires signed-request controls (service identifiers and tokens plus HMAC signatures, timestamps, and nonces to prevent replay). These constraints are not merely “API design choices.” They are the mechanism by which LiNKautowork enforces that every workflow run is attributable, tenant-scoped, and reconstructable, and that no external or internal caller can bypass governance by directly invoking arbitrary webhooks.

This contract continues through observability and interoperability. The gateway writes canonical audit telemetry for each execution to Supabase through controlled write paths, and it publishes events into the shared event plane using the canonical `aios.*` subject taxonomy (with an optional internal mirror subject family for LiNKautowork-specific consumers). As a result, the LiNKaios↔LiNKautowork interface is not only “trigger and respond.” It is “trigger, verify, execute, audit, and broadcast,” with each stage enforced as part of the system’s production posture.
