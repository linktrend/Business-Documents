...
The LiNKsites PRD documents are treated here as detailed supporting sources for the technical stack, CMS structure, template system, deployment assumptions, and the website factory’s current productized direction. They define the federated monorepo, the use of Payload CMS with Supabase, multi-site scoping, manufacturing hooks from the CMS into n8n workflows, standardized layouts for corporate and landing page formats, trilingual support, and the deployment assumptions around Ubuntu, Docker, and Cloudflare.
...
The term template refers not only to visual skins or design presets, but to pre-generated governed starting architectures that agents can select and extend rather than building from scratch. This is consistent with the broader IDP doctrine, where standardized templates and starter kits exist to reduce ambiguity and increase throughput, and with the LiNKsites PRD, which defines sub-template registries and industry-specific overrides inside the platform’s technical stack.

The term CMS refers to the centralized content command system of LiNKsites, implemented through the linksites-cms repository and using Payload CMS integrated with Supabase. It is not merely a page editor. It is a fleet-level administrative control surface governing content, page structures, site-scoped data, and update triggers across many sites. The LiNKsites PRD states explicitly that the CMS is a single centralized administrative interface managing content for the entire fleet of websites, and that multi-site scoping and manufacturing hooks are enforced within it.  ￼
...
This definition requires several clarifications. First, LiNKsites is not simply a repository of reusable code. Reusable code is one component of the system, but the platform also includes the centralized CMS, the data layer, the hosting and deployment conventions, the content synchronization mechanisms, the update triggers, the template registry, and the operational contracts that agents use when producing sites. The LiNKsites PRD confirms this by defining a federated architecture with at least two primary repositories, one for the front-end templates and one for the CMS, and by identifying a single centralized administrative interface, multi-site scoping, manufacturing hooks, and deployment assumptions as part of the technical stack.  ￼
